...
class VarianceUnificationSet(private val randomVariableScope: RandomVariableScope) : MutableSet<Unification> {
    ...
    override fun contains(element: Unification): Boolean = grouping.contains(element.variableValues)
    override fun containsAll(elements: Collection<Unification>): Boolean = elements.all { it in this }
    override fun isEmpty(): Boolean = size == 0
    override fun iterator(): MutableIterator<Unification> = MappingMutableIterator(grouping.iterator()) { (vars, _) -> Unification(vars) }
    override fun add(element: Unification): Boolean = grouping.set(element.variableValues, Unit) == null
    override fun addAll(elements: Collection<Unification>): Boolean = elements.map { add(it) }.any()
    override fun clear() {
        grouping.clear()
    }
    override fun remove(element: Unification): Boolean = grouping.remove(element.variableValues) != null
    ...
    override fun retainAll(elements: Collection<Unification>): Boolean {
        ...
        groupingElements@while (iterator.hasNext()) {
            ...
            retainers@while (toRetainIterator.hasNext()) {
                ...
                if (toRetainElement.variableValues.equalsStructurally(element.variableValues, randomVariableScope)) {
