...
class LexerIterator(givenSource: Iterator<Char>, initialSourceLocation: SourceLocation) : Iterator<Token> {
    ...
    private fun collectQuoted(startToken: OperatorToken): Pair<String, SourceLocationRange> {
        ...
        while (true) {
            if (!source.hasNext()) {
                throw ParseException.ofSingle(UnexpectedEOFError("Unexpected EOF in string starting at ${startToken.location.start}"))
            }
            ...
            if (possibleEscapeChar.first == ESCAPE_CHARACTER) {
                ...
                if (!source.hasNext()) {
                    source.rollback() // B
                    source.rollback() // A
                    throw ParseException.ofSingle(UnexpectedEOFError("Unexpected EOF after escape character in ${possibleEscapeChar.second}"))
                }
            ...
            if (possibleEndingOperatorText == null) {
                ...
                source.rollback() // B
                source.rollback() // A
                throw ParseException.ofSingle(UnexpectedEOFError("Failed to parse string starting at ${startToken.location.start}: unexpected EOF, expected ${startToken.operator}"))
            }
